//
//protoc -I=./pkg/order/pb --go_out=. --go-grpc_out=. order.proto
//ls ./pkg/order/pb/order.pb.go | xargs -n1 -IX bash -c 'sed s/// X > X.tmp && mv X{.tmp,}'
//ls ./pkg/order/pb/order_grpc.pb.go | xargs -n1 -IX bash -c 'sed s/// X > X.tmp && mv X{.tmp,}'
