...
//ls ./pkg/auth/pb/auth.pb.go | xargs -n1 -IX bash -c 'sed s/// X > X.tmp && mv X{.tmp,}'
//ls ./pkg/auth/pb/auth_grpc.pb.go | xargs -n1 -IX bash -c 'sed s/// X > X.tmp && mv X{.tmp,}'

// Define the request
//protoc -I=./ --go_out=./ ./account.proto
//ls *.pb.go | xargs -n1 -IX bash -c 'sed s/// X > X.tmp && mv X{.tmp,}'
